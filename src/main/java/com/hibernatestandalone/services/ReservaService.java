...
public class ReservaService extends AbstractService {
    ...
    public List<Reserva> getReservasPorHabitacion(Long habitacionId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            String hql = """
                FROM Reserva r 
                WHERE r.habitacion.id = :habitacionId 
                AND (r.estado = :estadoConfirmada OR r.estado = :estadoCheckIn)
            """;
            Query<Reserva> query = session.createQuery(hql, Reserva.class);
            query.setParameter("habitacionId", habitacionId);
            query.setParameter("estadoConfirmada", EstadoReserva.CONFIRMADA);
            query.setParameter("estadoCheckIn", EstadoReserva.CHECK_IN);
            return query.list();
        }
    }
